...
### Vivado version: 2015.4 (installed in: /opt/Xilinx/Vivado/2015.4)
...
#expect -c 'spawn -noecho vivado -mode tcl; expect {%} {send [exec cat scripts/setenv.tcl scripts/createELFs.tcl scripts/exit.tcl]}; interact'
expect -c 'spawn -noecho vivado -mode tcl; expect %; send [exec cat scripts/setenv.tcl scripts/createELFs.tcl]; expect "Invoking scanner config builder on project"; send "exit\r"; interact'
rm -rfv vivado*
echo -e "If no error message is observed, ELFs should have been created in project/sw and ../../OUTPUTS"
