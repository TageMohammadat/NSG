...
#define INTC_DEVICE_INTR_ID	XPAR_MICROBLAZE_2_AXI_INTC_KTH_AXI_MESH_2D_NOSTRUM_TDN_2X2X1_0_NOC_IRQ_2_INTR
#define INTC_DEVICE_INT_ID	XPAR_MICROBLAZE_2_AXI_INTC_KTH_AXI_MESH_2D_NOSTRUM_TDN_2X2X1_0_NOC_IRQ_2_INTR
#define INTC_DEVICE_INT_MASK	XPAR_KTH_AXI_MESH_2D_NOSTRUM_TDN_2X2X1_0_NOC_IRQ_2_MASK
#include "xaxipmon.h"
#define PERFMON_ID XPAR_AXI_PERF_MON_2_DEVICE_ID
#define PERFMON_BASEADDR XPAR_AXI_PERF_MON_2_BASEADDR
#define CYCLES_PER_uS XPAR_AXI_TIMER_2_CLOCK_FREQ_HZ/1000000
#define CYCLES_PER_mS XPAR_AXI_TIMER_2_CLOCK_FREQ_HZ/1000
#define CYCLES_PER_Sec XPAR_AXI_TIMER_2_CLOCK_FREQ_HZ
...
void p2_init(void)
{
	...
	XAxiPmon_ResetMetricCounter(&perfmon);
		...
		//Time slice 2--------------------------------------------------
		// compute GY - Microblaze 2
		//compute_GY(tokens_GetPixel, tokensWrite_GY);
		//tokens_GetPixel=@p2_in0->x[0];tokensWrite_GY=@p2_out3->x[0]
		while (1){if (InterruptProcessed) break;}InterruptProcessed=FALSE;//xil_printf("\ninterrupted\n");
		while (NOC_RNI_RECV_CHANNEL_STATUS(NOC_RNI_BASE,recv_channel_p2_from_p0)==0);NOC_RNI_CLEAR(NOC_RNI_BASE,recv_channel_p2_from_p0);
	        //xil_printf("@MB2, Got: %d\n",(int)p2_in0->x[0]);
		compute_GY((int*)p2_in0->x, (int*)p2_out3->x);
		...
		//printf("Finished producing initial tokens to P3 from P2\n");
		//Give enough time for P0 to produce tokens
		while ( samples < CYCLES_PER_Sec ) 
		{
		samples = XAxiPmon_SampleMetrics(&perfmon);
...
void p2_main(void)
{
	...
	int led=2;  int microseconds; int milliseconds;
	for(;image_y < image_height;image_y++) { //in non-ARM core main loop
		...
		samples=cur_samples-prev_samples;
                NOC_RNI_CLEAR_SYNCHRONIZER_FLAG(NOC_RNI_BASE,recv_channel_p2_from_p0);
		NOC_RNI_SEND(NOC_RNI_BASE,send_channel_p2_to_p3,p2_out3_priority,80);//p2_out3_msg_len);
		//Some rubbish messages
		NOC_RNI_SEND(NOC_RNI_BASE,send_channel_p2_to_p2,0,80);
		NOC_RNI_SEND(NOC_RNI_BASE,send_channel_p2_to_p1,0,80);
		NOC_RNI_SEND(NOC_RNI_BASE,send_channel_p2_to_p0,0,80);
    	   }
    	sleep(2);
	xil_printf("compute_GY/uBlaze takes %d - %d= %d ticks\n",prev_samples,cur_samples,samples);
