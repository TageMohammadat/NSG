...
void p1_init(void)
{
	...
	(*p1_out0)=0xFFF1; NOC_RNI_SEND(NOC_RNI_BASE,0,p1_pid,p0_pid,send_channel_p1_to_p0,sizeof(int)/4);//send 0xFFF1 as signature from P1
};
...
void p1_main(void)
{
	...
	*p1_out0=1; int led=1;
	   while(1) // Loop forever
	   {
		...
		NOC_RNI_SEND(NOC_RNI_BASE,0,p1_pid,p0_pid,send_channel_p1_to_p0,sizeof(int)/4);
		//display the toggling least significant digit of led value at the LED
		IOWR(PIO_0_BASE, 0,*p1_in0); //led++ instead of *p1_in0
